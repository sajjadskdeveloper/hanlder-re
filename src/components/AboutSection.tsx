...
const AboutSection = () => {
  ...
  return (
    <section id="about" className="py-32 bg-gradient-to-br from-slate-50 via-white to-slate-100 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
         ...
         <div className={`text-center mb-16 sm:mb-20 lg:mb-24 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
           <div className="inline-flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-slate-800 to-slate-900 text-white rounded-full text-xs sm:text-sm font-medium mb-6 sm:mb-8 shadow-lg">
             <div className="w-1.5 sm:w-2 h-1.5 sm:h-2 bg-blue-400 rounded-full mr-2 sm:mr-3 animate-pulse"></div>
             ABOUT HANDLER
           </div>
           <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-slate-900 mb-6 sm:mb-8 leading-tight px-4">
           ...
           <div className="w-20 sm:w-24 md:w-28 lg:w-32 h-0.5 bg-gradient-to-r from-blue-600 via-purple-600 to-blue-600 mx-auto rounded-full shadow-lg"></div>
         </div>

        {/* Main Content Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center mb-32">
          {/* Left Content */}
          <div className={`space-y-10 transition-all duration-1000 delay-200 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            ...
            <div className="grid grid-cols-2 gap-6">
              {stats.map((stat, index) => (
                <div key={index} className="text-center p-8 bg-white rounded-2xl shadow-sm border border-slate-200 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
          ...
          <div className={`relative transition-all duration-1000 delay-400 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
            ...
            <div className="relative z-10 group">
              <div className="relative overflow-hidden rounded-3xl shadow-2xl">
                <img
                  ...
                  className="w-full h-[600px] object-cover transform group-hover:scale-110 transition-transform duration-700 ease-out"
                />
                ...
                <div className="absolute bottom-8 left-8 right-8 text-white">
                  <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
                    <div className="flex items-center mb-3">
                      ...
                      <span className="text-sm font-medium text-blue-200 uppercase tracking-wide">New York City</span>
                    </div>
            ...
            {/* Sophisticated Background Elements */}
            <div className="absolute -bottom-10 -right-10 w-full h-full bg-gradient-to-br from-slate-800 via-slate-700 to-slate-900 rounded-3xl -z-10 transform rotate-3 shadow-2xl"></div>
            <div className="absolute -bottom-5 -right-5 w-full h-full bg-gradient-to-br from-blue-600/20 to-purple-600/20 rounded-3xl -z-10 transform rotate-1"></div>
            
            {/* Floating Decorative Elements */}
            <div className="absolute -top-12 -left-12 w-40 h-40 bg-gradient-to-br from-blue-500/30 to-purple-600/30 rounded-full -z-10 animate-pulse blur-xl"></div>
            <div className="absolute top-20 -right-8 w-24 h-24 bg-gradient-to-br from-emerald-500/20 to-teal-600/20 rounded-full -z-10 animate-bounce opacity-60"></div>
            <div className="absolute bottom-20 -left-6 w-16 h-16 bg-gradient-to-br from-orange-500/20 to-red-600/20 rounded-full -z-10 animate-ping opacity-40"></div>
            
            {/* Professional Border Accent */}
            <div className="absolute inset-0 rounded-3xl border-2 border-white/10 -z-10"></div>
          </div>
        </div>

        {/* Investment Portfolio Section */}
        <div className={`bg-white p-12 rounded-3xl shadow-xl mb-32 border border-slate-200 transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-32">
          {businessLines.map((line, index) => (
            <div key={index} className={`bg-white p-8 rounded-2xl shadow-sm border border-slate-200 hover:shadow-xl transition-all duration-500 transform hover:-translate-y-2 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: `${index * 100}ms` }}>
              <div className={`w-16 h-16 bg-gradient-to-br ${line.color} rounded-2xl flex items-center justify-center mb-6 shadow-lg`}>
                <div className="text-white">
                  {line.icon}
                </div>
              </div>
              <h4 className="text-2xl font-bold text-slate-900 mb-4">{line.title}</h4>
        ...
        <div className={`bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6 sm:p-8 md:p-12 lg:p-16 rounded-2xl sm:rounded-3xl shadow-2xl mb-16 sm:mb-24 lg:mb-32 text-white relative overflow-hidden transition-all duration-1000 delay-400 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(59,130,246,0.1)_0%,transparent_70%)]"></div>
          <div className="relative z-10 text-center">
            <h3 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-4 sm:mb-6 lg:mb-8 leading-tight px-2">
              MORE THAN BRICKS AND MORTAR,
              <br />
              <span className="bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">RELATIONSHIPS MATTER</span>
            </h3>
            <div className="w-20 sm:w-24 md:w-28 lg:w-32 h-1 bg-gradient-to-r from-blue-400 to-purple-400 mx-auto rounded-full mb-6 sm:mb-8 lg:mb-12"></div>
            <p className="text-base sm:text-lg md:text-xl text-slate-300 max-w-4xl lg:max-w-5xl mx-auto leading-relaxed font-light px-4">
        ...
        <div id="tenantServices1" className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-32">
          {/* Left Content */}
          <div className={`space-y-8 transition-all duration-1000 delay-500 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            <div>
              <h3 className="text-4xl font-bold text-slate-900 mb-6">Tenant Representation</h3>
            ...
            <div className="space-y-6">
              {tenantServices.map((service, index) => (
                <div key={service.id} className="flex items-start space-x-4">
                  ...
                  <span className="text-slate-700 font-medium">{service.text}</span>
                </div>
              ))}
            </div>
          ...
          <div className={`bg-white p-10 rounded-3xl shadow-xl border border-slate-200 transition-all duration-1000 delay-600 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
            ...
            <div className="mt-8 relative overflow-hidden rounded-2xl h-32">
              <img
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center text-white">
                  ...
                  <p className="text-sm font-medium">Premium Locations</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className={`text-center transition-all duration-1000 delay-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="bg-white p-16 rounded-3xl shadow-2xl max-w-5xl mx-auto border border-slate-200">
            <h3 className="text-4xl font-bold text-slate-900 mb-6">
              Ready to Experience the Handler Difference?
            </h3>
            <div className="w-24 h-1 bg-gradient-to-r from-blue-600 to-purple-600 mx-auto rounded-full mb-8"></div>
            <p className="text-xl text-slate-600 mb-12 max-w-3xl mx-auto font-light">
              Discover how our creative thinking and committed doing approach can transform your real estate experience
            </p>
            <div className="flex flex-col sm:flex-row gap-6 justify-center">
              <button className="bg-gradient-to-r from-slate-800 to-slate-900 text-white px-12 py-5 rounded-2xl font-semibold hover:shadow-xl transition-all duration-300 transform hover:scale-105 shadow-lg">
                Contact Us Today
              </button>
              <button className="border-2 border-slate-800 text-slate-800 px-12 py-5 rounded-2xl font-semibold hover:bg-slate-800 hover:text-white transition-all duration-300 shadow-lg">
                View Our Properties
              </button>
            </div>
          </div>
